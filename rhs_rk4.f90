module rhs_RK4
	...
	subroutine halo_swap_WS(ws_in)
		...
		if (INC_MAG_FIELDS) then
			do m = 1,3
				do n = 1,NGHOST
					call halo_swap_real(ws_in%MAGNETIC(f)%GRID_R,n)
				end do
			end do
		end if
	end subroutine
	...
	subroutine halo_swap_complex(kk,n)
		...
		call MPI_sendrecv(kk(ex-NGHOST-n+1,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1), MPI_DOUBLE_COMPLEX, cart_shift(0)%rd,0,&
			kk(sx+NGHOST-n,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1),MPI_DOUBLE_COMPLEX, cart_shift(0)%rs, 0, MPI_COMM_GRID,MPISTAT,IERR)
		...
		call MPI_sendrecv(kk(sx+NGHOST+n-1,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1), MPI_DOUBLE_COMPLEX, cart_shift(0)%rs,1,&
			kk(ex-NGHOST+n,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1),MPI_DOUBLE_COMPLEX, cart_shift(0)%rd, 1, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ", IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,ey-NGHOST-n+1,sz:ez),(ex-sx+1)*(ez-sz+1), MPI_DOUBLE_COMPLEX, cart_shift(1)%rd,2,&
			kk(sx:ex,sy+NGHOST-n,sz:ez),(ex-sx+1)*(ez-sz+1),MPI_DOUBLE_COMPLEX, cart_shift(1)%rs, 2, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ", IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,sy+NGHOST+n-1,sz:ez),(ex-sx+1)*(ez-sz+1), MPI_DOUBLE_COMPLEX, cart_shift(1)%rs,3,&
			kk(sx:ex,ey-NGHOST+n,sz:ez),(ex-sx+1)*(ez-sz+1),MPI_DOUBLE_COMPLEX, cart_shift(1)%rd, 3, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ",IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,sy:ey,ez-NGHOST-n+1),(ex-sx+1)*(ey-sy+1), MPI_DOUBLE_COMPLEX, cart_shift(2)%rd,4,&
			kk(sx:ex,sy:ey,sz+NGHOST-n),(ex-sx+1)*(ey-sy+1),MPI_DOUBLE_COMPLEX, cart_shift(2)%rs, 4, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ",IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
	...
	subroutine halo_swap_real(kk,n)
		...
		call MPI_sendrecv(kk(ex-NGHOST-n+1,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1), MPI_DOUBLE, cart_shift(0)%rd,0,&
			kk(sx+NGHOST-n,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1),MPI_DOUBLE, cart_shift(0)%rs, 6, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ",IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx+NGHOST+n-1,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1), MPI_DOUBLE, cart_shift(0)%rs,1,&
			kk(ex-NGHOST+n,sy:ey,sz:ez),(ey-sy+1)*(ez-sz+1),MPI_DOUBLE, cart_shift(0)%rd, 7, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ", IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,ey-NGHOST-n+1,sz:ez),(ex-sx+1)*(ez-sz+1), MPI_DOUBLE, cart_shift(1)%rd,2,&
			kk(sx:ex,sy+NGHOST-n,sz:ez),(ex-sx+1)*(ez-sz+1),MPI_DOUBLE, cart_shift(1)%rs, 8, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ", IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,sy+NGHOST+n-1,sz:ez),(ex-sx+1)*(ez-sz+1), MPI_DOUBLE, cart_shift(1)%rs,3,&
			kk(sx:ex,ey-NGHOST+n,sz:ez),(ex-sx+1)*(ez-sz+1),MPI_DOUBLE, cart_shift(1)%rd, 9, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ",IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,sy:ey,ez-NGHOST-n+1),(ex-sx+1)*(ey-sy+1), MPI_DOUBLE, cart_shift(2)%rd,4,&
			kk(sx:ex,sy:ey,sz+NGHOST-n),(ex-sx+1)*(ey-sy+1),MPI_DOUBLE, cart_shift(2)%rs, 10, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ",IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
		...
		call MPI_sendrecv(kk(sx:ex,sy:ey,sz+NGHOST+n-1),(ex-sx+1)*(ey-sy+1), MPI_DOUBLE, cart_shift(2)%rs,5,&
			kk(sx:ex,sy:ey,ez-NGHOST+n),(ex-sx+1)*(ey-sy+1),MPI_DOUBLE, cart_shift(2)%rd, 100, MPI_COMM_GRID,MPISTAT,IERR)
		if(IERR .ne. MPI_SUCCESS) then
			write(6,*) "Error running MPI_sendrecv on Rank: ", RANK, ". Error code: ", IERR
			write(6,*) "Something has gone wrong... Quitting."
			CALL EXIT(1)
	...
	subroutine RK4_renormalise_fluid(gt,tgt)
		...
		tgt(sx+NGHOST:ex-NGHOST,sy+NGHOST:ey-NGHOST,sz+NGHOST:ez-NGHOST) = gt(sx+NGHOST:ex-NGHOST, &
					sy+NGHOST:ey-NGHOST,sz+NGHOST:ez-NGHOST)*conjg(gt(sx+NGHOST:ex-NGHOST, &
					sy+NGHOST:ey-NGHOST,sz+NGHOST:ez-NGHOST))
