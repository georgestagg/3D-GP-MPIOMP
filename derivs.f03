...
contains
	...
	COMPLEX*16 function curlcurlx_magnetic(ws_in,i,j,k)
		use params
		implicit none
		integer :: i,j,k
		type(workspace_t), intent(in) :: ws_in

		curlcurlx_magnetic = (4.0d0*MBC(ws_in,i,j,k,1) - MBC(ws_in,i,j+1,k,1) - MBC(ws_in,i,j-1,k,1) - MBC(ws_in,i,j,k+1,1) &
							- MBC(ws_in,i,j,k-1,1) &
							+ MBC(ws_in,i+1,j,k,2) - MBC(ws_in,i,j,k,2) - MBC(ws_in,i+1,j-1,k,2) + MBC(ws_in,i,j-1,k,2) &
							+ MBC(ws_in,i+1,j,k,3) - MBC(ws_in,i,j,k,3) - MBC(ws_in,i+1,j,k-1,3) + MBC(ws_in,i,j,k-1,3))/(DSPACE**2.0d0)
	end function

	COMPLEX*16 function curlcurly_magnetic(ws_in,i,j,k)
		use params
		implicit none
		integer :: i,j,k
		type(workspace_t), intent(in) :: ws_in

		curlcurly_magnetic = (4.0d0*MBC(ws_in,i,j,k,2) - MBC(ws_in,i,j,k+1,2) - MBC(ws_in,i,j,k-1,2) - MBC(ws_in,i+1,j,k,2) &
							- MBC(ws_in,i-1,j,k,2) &
							+ MBC(ws_in,i,j+1,k,3) - MBC(ws_in,i,j,k,3) - MBC(ws_in,i,j+1,k-1,3) + mBC(ws_in,i,j,k-1,3) &
							+ MBC(ws_in,i,j+1,k,1) - MBC(ws_in,i,j,k,1) - MBC(ws_in,i-1,j+1,k,1) + MBC(ws_in,i-1,j,k,1))/(DSPACE**2.0d0)
	end function

	COMPLEX*16 function curlcurlz_magnetic(ws_in,i,j,k)
		use params
		implicit none
		integer :: i,j,k
		type(workspace_t), intent(in) :: ws_in

		curlcurlz_magnetic = (4.0d0*MBC(ws_in,i,j,k,3) - MBC(ws_in,i+1,j,k,3) - MBC(ws_in,i-1,j,k,3) - MBC(ws_in,i,j+1,k,3) &
							- MBC(ws_in,i,j-1,k,3) &
							+ MBC(ws_in,i,j,k+1,1) - MBC(ws_in,i,j,k,1) - MBC(ws_in,i-1,j,k+1,1) + MBC(ws_in,i-1,j,k,1) &
							+ MBC(ws_in,i,j,k+1,2) - MBC(ws_in,i,j,k,2) - MBC(ws_in,i,j-1,k+1,2) + MBC(ws_in,i,j-1,k,2))/(DSPACE**2.0d0)
	end function

	complex*16 function MBC(ws_in,i,j,k,dir)
		use params
		implicit none
		integer :: dir
		...
		!Reflective
		if(i == NX+1 .and. BCMX == 0) ii=NX
		if(i == 0   .and. BCMX == 0) ii=1
		if(j == NY+1 .and. BCMY == 0) jj=NY
		if(j == 0   .and. BCMY == 0) jj=1
		if(k == NZ+1 .and. BCMZ == 0) kk=NZ
		if(k == 0   .and. BCMZ == 0) kk=1
		
		MBC = ws_in%MAGNETIC(dir)%GRID(ii,jj,kk)
	...
	complex*16 function BC(field_in,i,j,k,dir)
		...
		if(j == NY+1 .and. BCY==3) then
		  BC = BC*exp(GX(ii)*((NY-1)*DSPACE)*BZ - GZ(kk)*((NY-1)*DSPACE)*BX&
					  -EYE*PI*NVORTALL(3,field_in%field_number)*GX(ii)/((NX-1)*DSPACE)&
