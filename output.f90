...
Contains
  ...
  subroutine write_wf_file_RK4(rt)
    ...
    do f = 1, FLUIDS
      r = NF90_put_var(ncdf_id, f_re_id(f), DBLE(WS%FLUID(f)%GRID( &
                                                 sx + NGHOST:ex - NGHOST, &
                                                 sy + NGHOST:ey - NGHOST, &
                                                 sz + NGHOST:ez - NGHOST)), istarting, icount)
      call handle_err(r)
      r = NF90_put_var(ncdf_id, f_im_id(f), DIMAG(WS%FLUID(f)%GRID( &
                                                  sx + NGHOST:ex - NGHOST, &
                                                  sy + NGHOST:ey - NGHOST, &
                                                  sz + NGHOST:ez - NGHOST)), istarting, icount)
      call handle_err(r)
    ...
    r = NF90_put_var(ncdf_id, pot_id, POT( &
                     sx + NGHOST:ex - NGHOST, &
                     sy + NGHOST:ey - NGHOST, &
                     sz + NGHOST:ez - NGHOST), istarting, icount)
    call handle_err(r)
  ...
  subroutine read_wf_file_RK4(fname)
    ...
    icount(3) = ez - sz - 1
    do f = 1, FLUIDS
      r = NF90_get_var(rwf_ncid, rwf_re_id(f), &
                       realgrid(sx + 1:ex - 1, sy + 1:ey - 1, sz + 1:ez - 1), start=istarting, count=icount)
      call handle_err(r)
      r = NF90_get_var(rwf_ncid, rwf_im_id(f), &
                       imaggrid(sx + 1:ex - 1, sy + 1:ey - 1, sz + 1:ez - 1), start=istarting, count=icount)
      call handle_err(r)
      WS%FLUID(f)%GRID = realgrid + EYE*imaggrid
    end do

    r = NF90_get_var(rwf_ncid, rwf_pot_id, &
                     POT(sx + 1:ex - 1, sy + 1:ey - 1, sz + 1:ez - 1), start=istarting, count=icount)
    call handle_err(r)
